...
QHash<QString, QString> DeviceManagement::getDeviceInformation() {
    ...
    QDomElement deviceInfo = newElement("GetDeviceInformation");
    deviceInfo.setAttribute("xmlns", "http://www.onvif.org/ver10/device/wsdl");

    msg->appendToBody(deviceInfo);
...
Capabilities *DeviceManagement::getCapabilities()
{
    Capabilities *capabilities = NULL;
    Message *msg = newMessage();
...
void DeviceManagement::getScopes()
{
    Message *msg = newMessage();
    QDomElement scope = newElement("GetScopes");
    scope.setAttribute("xmlns", "http://www.onvif.org/ver10/device/wsdl");

    msg->appendToBody(scope);
    sendMessage(msg);
}

void DeviceManagement::getNetworkInterface()
{
    Message *msg = newMessage();
    QDomElement scope = newElement("GetNetworkInterfaces");
    scope.setAttribute("xmlns", "http://www.onvif.org/ver10/device/wsdl");
...
void DeviceManagement::getDNS()
{
    Message *msg = newMessage();
    QDomElement dns = newElement("GetDNS");
    dns.setAttribute("xmlns", "http://www.onvif.org/ver10/device/wsdl");

    msg->appendToBody(dns);
    sendMessage(msg);
}

QHash<QString, QString> DeviceManagement::getServices()
{
    QHash<QString, QString> serviceInfo;

    Message *msg = newMessage();
    QDomElement serviceEl = newElement("GetServices");
    serviceEl.setAttribute("xmlns", "http://www.onvif.org/ver10/device/wsdl");

    serviceEl.appendChild(newElement("IncludeCapability", "false"));

    msg->appendToBody(serviceEl);
    MessageParser *result = sendMessage(msg);
    if(result)
    {
        QXmlQuery *query = result->query();
        query->setQuery(result->nameSpace()+"doc($inputDocument)//tds:Service");
        QXmlResultItems items;
        query->evaluateTo(&items);
        QXmlItem item = items.next();
        QDomDocument doc;
        QString value, videoSourceNode;

        while(!item.isNull()){
            query->setFocus(item);

            query->setQuery(result->nameSpace()+".");
            query->evaluateTo(&videoSourceNode);
            doc.setContent(videoSourceNode);

            query->setQuery(result->nameSpace()+"./tds:Namespace/string()");
            query->evaluateTo(&value);
            QString nameSpaceStr = value.trimmed();

            query->setQuery(result->nameSpace()+"./tds:XAddr/string()");
            query->evaluateTo(&value);
            QString serviceUri = value.trimmed();

            if(nameSpaceStr == "http://www.onvif.org/ver10/media/wsdl")
                serviceInfo.insert("Media", serviceUri);
            else if(nameSpaceStr == "http://www.onvif.org/ver10/events/wsdl")
                serviceInfo.insert("Event", serviceUri);
