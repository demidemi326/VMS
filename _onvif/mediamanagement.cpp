...
VideoSourceConfigurations *MediaManagement::getVideoSourceConfigurations()
{
    ...
    msg->appendToBody(newElement("GetVideoSourceConfigurations"));
...
VideoEncoderConfigurations *MediaManagement::getVideoEncoderConfigurations()
{
    ...
    Message *msg = newMessage();
    msg->appendToBody(newElement("GetVideoEncoderConfigurations"));
    MessageParser *result = sendMessage(msg);
    if(result != NULL){
...
Profiles *MediaManagement::getProfiles()
{
    ...
    Message *msg = newMessage();
    QDomElement profileEle = newElement("GetProfiles");
    profileEle.setAttribute("xmlns", "http://www.onvif.org/ver10/media/wsdl");

    msg->appendToBody(profileEle);
    MessageParser *result = sendMessage(msg);
    if(result != NULL){
...
StreamUri *MediaManagement::getStreamUri(QString profileTokenStr)
{
    ...
    QDomElement profileToken = newElement("ProfileToken",profileTokenStr);
...
QStringList MediaManagement::getVideoSource()
{
    QStringList videoSources;

    Message *msg = newMessage();
    QDomElement videoSourceEl = newElement("GetVideoSources");
    videoSourceEl.setAttribute("xmlns", "http://www.onvif.org/ver10/media/wsdl");

    msg->appendToBody(videoSourceEl);
    MessageParser* result = sendMessage(msg);
    if(result)
    {
        QXmlQuery *query = result->query();
        query->setQuery(result->nameSpace()+"doc($inputDocument)//trt:VideoSources");
        QXmlResultItems items;
        query->evaluateTo(&items);
        QXmlItem item = items.next();
        QDomDocument doc;
        ...
        while(!item.isNull()){
            ...
            QDomNodeList itemNodeList = doc.elementsByTagName("trt:VideoSources");
            for(int i=0; i<itemNodeList.size();i++)
            {
                value = itemNodeList.at(i).toElement().attribute("token");
                videoSources.append(value.trimmed());
            }
